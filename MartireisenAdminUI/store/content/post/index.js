...
const actions = {
  ...
  async get ({ commit }, { page }) {
    ...
    const res = await this.$axios.get(endpoints['content_post'],{
      params : {
        page: page.current,
        limit: page.pageSize
      }
  ...
  async getTree ({ commit }, { page }) {

    const res = await this.$axios.get(endpoints['content_post_category']+'/tree',{
      params : {
        page: page.current,
        limit: page.pageSize
      }
  ...
  async delete({ commit, dispatch }, { id }) {
    ...
    if (id.length === 0) {
      ...
      for (let i = 0; i < id.length; i++) {
        const { data } =   await this.$axios.delete(endpoints['content_post']+'/'+id[i]);
        if (data.status) {
