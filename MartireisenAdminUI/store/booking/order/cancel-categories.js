...
const actions = {
  ...
  async get({commit}, {page}) {
      ...
      const res = await this.$axios.get(endpoints['ecommerce_order_cancel_categories'], {
        params: {
          page: page.current,
          limit: page.pageSize
        }
  ...
  async delete({commit, dispatch}, {id}) {
    ...
    if (id.length === 0) {
      ...
      for (let i = 0; i < id.length; i++) {
        const {data} = await this.$axios.delete(endpoints['ecommerce_order_cancel_categories'] + '/' + id[i]);
        if (data.status) {
