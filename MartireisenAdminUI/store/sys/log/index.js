...
const actions = {
  ...
  async get({commit}, {page}) {
    ...
    const res = await this.$axios.get(endpoints['log'], {
      params: {
        page: page.current,
        limit: page.pageSize,
      }
  ...
  async getFilteredData({commit}, {page, searchData}) {
    ...
    const res = await this.$axios.get(endpoints['log'], {
      params: {
        ...
        action: searchData.action_id,
        module: searchData.module_id,
        admin_id: searchData.admin_id,
  ...
  async delete({commit, dispatch}, {id}) {
    ...
    if (id.length === 0) {
      ...
      for (let i = 0; i < id.length; i++) {
        const {
          data
        } = await this.$axios.delete(endpoints['log'] + '/' + id[i]);
        if (data.status) {
