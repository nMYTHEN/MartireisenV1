<template>
  <div class="container-fluid">
    <div class="air__utils__heading">
      <h5 class="float-left">Seo Text</h5>
...
export default {
  ...
  mounted() {
    this.$store.dispatch("homepage/seo/text/get", { page: 1 });
  },
  computed: {
    rowSelection() {
    ...
    table() {
      return this.$store.state.homepage.seo.text.table;
    }
  },
  ...
  methods: {
    ...
    handleTableChange(pagination, filters, sorter) {
      this.$store.dispatch("homepage/seo/text/get", {
        page: pagination
      });
    },
    handleClickAction(name) {
      switch (name) {
        case "refresh":
          this.$store.dispatch("homepage/seo/text/refresh");
          break;

        case "new":

          this.$router.push({
              path: '/homepage/seo/text/save'
          })
          break;

        case "delete":
          this.$store.dispatch('homepage/seo/text/delete', {
