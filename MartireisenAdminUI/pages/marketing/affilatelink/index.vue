...
<script>
  ...
  export default {
    ...
    mounted() {
      this.$store.dispatch("marketing/affilatelink/get", {page: 1});
    },
    computed: {
      table() {
        return this.$store.state.marketing.affilatelink.table;
      },
    },
    ...
    methods: {
      handleTableChange(pagination, filters, sorter, filtered, data) {
        if (filtered) {
          this.$store.dispatch("marketing/affilatelink/getFilteredData", {searchData: data, page: pagination});

        } else {
          this.$store.dispatch("marketing/affilatelink/get", {page: pagination});
        }
      },
      handleClickAction(name) {
        switch (name) {
          case "refresh":
            this.$store.dispatch("marketing/affilatelink/refresh");
            break;

          case "new":
            this.$router.push({
            path: "/marketing/affilatelink/0"
          });
            break;
      ...
      deleteRecord(id) {
        this.$store.dispatch("marketing/affilatelink/delete", {
          id: [id]
        });
      },
    }
