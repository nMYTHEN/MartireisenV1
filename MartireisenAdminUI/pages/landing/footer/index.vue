<template>
  <div class="container-fluid">
    <div class="air__utils__heading">
      <h5 class="float-left">{{ $t('pages.landing_link.title')}}</h5>
    ...
    >
      ...
      <span slot="type" slot-scope="record">
          <span v-if="record.value == 2">Lastminute</span>
          <span v-if="record.value == 1">Pauschal</span>
          <span v-if="record.value == 0">Standart</span>
      </span>
...
<script>
  ...
  export default {
    ...
    mounted() {
      this.$store.dispatch("landing/footer/get", {page: 1});
    ...
    computed: {
      ...
      table() {
        return this.$store.state.landing.footer.table;
      }
    },
    ...
    methods: {
      ...
      handleTableChange(pagination, filters, sorter) {
        this.$store.dispatch("landing/footer/get", {
          page: pagination
        });
      },
      handleClickAction(name) {
        switch (name) {
          case "refresh":
            this.$store.dispatch("landing/footer/refresh");
            break;
          ...
          case "delete":
            this.$store.dispatch('landing/footer/delete', {
      ...
      onSubmit() {
        ...
        if (this.form.id) {
          ...
          this.$axios.put("/landing/footer/" + this.form.id + extra, this.form)
            .then(response => {
              this.$store.dispatch("landing/footer/refresh");
          ...
          this.$axios.post("/landing/footer/", this.form)
            .then(response => {
              this.$store.dispatch("landing/footer/refresh");
              this.onResponse(response);
            })
