        ...
        <a-form class="form-vertical" layout="vertical">
          ...
          ">
                  ...
                  <a-form-item :label-col="{ span: 24 }" :wrapper-col="{ span: 24 }">
                    <a-divider orientation="left">{{ $t('pages.contents.save.summary') }}</a-divider>
                    <textarea class="ant-input" v-model="form.summary"></textarea>
                  </a-form-item>
                  <a-form-item :label-col="{ span: 24 }" :wrapper-col="{ span: 24 }">
          ...
          <a-col :span="6">
            <a-card>
              <a-form-item :label-col="{ span: 24 }" :wrapper-col="{ span: 24 }" v-if="!isTranslate">
                <a-divider orientation="left">{{ $t('pages.contents.save.categories') }}</a-divider>
                <a-tree-select v-model="form.categories" :treeData="tree" treeCheckable
                  searchPlaceholder="Please select"></a-tree-select>
              </a-form-item>
...
export default {
  ...
  mounted() {
    this.$axios.get('/content/post/category/tree').then((r) => {
  ...
  methods: {
    ...
    fetch() {
      return this.$axios
        .get("/content/post/post/" + this.$route.params.id)
        .then(response => {
    ...
    onSubmit() {
      if (this.$route.params.id) {
        ...
        this.$axios
          .put("/content/post/post/" + this.$route.params.id + extra, this.form)
          .then(response => {
        ...
        this.$axios
          .post("/content/post/post", this.form)
          .then(response => {
            this.onResponse(response);
          })
          .catch(error => {
    ...
    onResponse(response) {
      ...
      if (result.action == "insert") {
        this.$router.push({
          path: "/content/contents/save/" + result.id
        });
      }
    },
